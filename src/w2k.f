*
* Modifications made by Mei-Ching Fok
*  September 23, 2011
*   - In FUNCTION EpotVal, define dplm and put it as the last argument of DLEGENDRE
*   - In SUBROUTINE SetModel, change SIND(Tilt) to SIN(Tilt)
*  September 26, 2011
*   - Make a ascii verision of w2k.dat. In subroutine SetModel, reading 
*     w2k_ascii.dat instead. READ(unit) are change to READ(unit,*).
*  January 22, 2015
*   - replace COMMON /SetW2kCoef/MaxL,MaxM,MaxN,Coef,BoundFit,pi,dpi
*     by      COMMON /SetW2kCoef/dpi,Coef,BoundFit,pi,MaxL,MaxM,MaxN
*
...
c       COMMON /SetW2kCoef/MaxL,MaxM,MaxN,Coef,BoundFit,pi,dpi
	COMMON /SetW2kCoef/dpi,Coef,BoundFit,pi,MaxL,MaxM,MaxN

	If(First)Then
        cfile='w2k.dat' !make sure correct ENDIAN type file is used.
	unit=99
c MCF   OPEN(UNIT=unit,FILE=cfile,STATUS='OLD',form='UNFORMATTED')
	OPEN(UNIT=unit,FILE='w2k_ascii.dat',STATUS='OLD')   
...
*
	...
	DOUBLE PRECISION  dPlm(0:10,0:10)        ! MCF  
...
c       COMMON /SetW2kCoef/MaxL,MaxM,MaxN,Coef,BoundFit,pi,dpi
	...
	IF(glat .GT. blat)THEN
	  ...
	  O=DC
	  ct=DCOS(x)
	  CALL DLegendre(ct,MaxL,MaxM,Plm,dPlm)         ! MCF
!Also find value at outer boundary at angle Pi, or cos(pi)=-1.
	  CALL DLegendre(-1.D0,MaxL,MaxM,OPlm,dPlm)     ! MCF
	  DO l=1,MaxL
